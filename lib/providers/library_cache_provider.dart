...
/// 每日推荐歌曲（首页）
final dailySongsProvider = FutureProvider.autoDispose<List<Song>>((ref) async {
  ref.keepAlive();
  final client = ref.watch(subsonicClientProvider);
  return await client.getRandomSongs(size: 9);
});
...
/// 电台列表（电台页）
final radioStationsProvider = FutureProvider.autoDispose<List<RadioStation>>((ref) async {
  ref.keepAlive();
  final client = ref.watch(subsonicClientProvider);
...
final genreSongsProvider = FutureProvider.autoDispose.family<List<Song>, String>((ref, genreName) async {
  ref.keepAlive();
  final client = ref.watch(subsonicClientProvider);
