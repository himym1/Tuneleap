...
def _artist_tokens(value: object) -> tuple[str, ...]:
    ...
    splitter = (
        _CJK_ARTIST_SEPARATOR_RE
        if _CJK_RE.search(artist)
        else _SPACED_ARTIST_SEPARATOR_RE
    )
    artist = splitter.sub("|", artist)
    return tuple(
        sorted({token for part in artist.split("|") if (token := normalize_text(part))})
    )
